        ...
        <div class="v-table-panel-items">
          <div class="v-table-panel-items__actions flex-wrap">
            <v-btn
              ...
              class="mb-2"
              v-show="
                button.type === 'sendEditable' ? adding.rows.length : true
              "
      ...
      <div class="v-table-wrap">
        <table id="mainTable" ref="mainTable">
          ...
          <tbody v-if="!loading && options.data.rows" class="v-table-body">
            ...
            />
            <tr :rowspan="options.head.length" class="tr-left-fixed">
              <td class="td-left-fixed" style="left: 0" :colspan="3">
                <template>
                  <p @click="createEditable">+ Добавить поле</p>
                </template>
              </td>
            </tr>
            <template v-for="(row, rowEditableIndex) in adding.rows">
              <tr
                v-if="adding.isShow"
                :class="[row.selected ? 'v-table-body-row--selected' : '']"
                :key="row.id"
                @contextmenu="openContext($event, row)"
                @click="openChildRow($event, row)"
                class="v-table-body-row"
                ref="cellItemsEditable"
              >
                <td
                  :style="{
                    width: cell.width,
                  ...
                  @dblclick="openEditableCell($event, row, cell)"
                  :key="cellIndex"
                >
                  <template
                    v-if="
                      cell.type === 'default' && cell.hasOwnProperty('editable')
                    "
                  >
                    <v-row>
                      <v-col
                        ...
                        class="field-col"
                        :class="[
                          cell.editable.field.type,
                          typeof cell.editable.field.isShow === 'object' &&
                          !cell.editable.field.isShow.value
                            ? 'isHide'
                            : '',
                        ]"
                        ref="fieldsRef"
                      >
                        <Autocomplete
                          v-if="showField('select', cell.editable.field)"
                          :field="cell.editable.field"
                          v-model="row.formData[cell.editable.field.name]"
                          :formData="formData"
                          ref="autocompleteRef"
                          @change="changeAutocomplete"
                          :class="[...cell.editable.field.class]"
                        />
                        <Autocomplete
                          v-else-if="
                            showField('autocomplete', cell.editable.field)
                          "
                          :field="cell.editable.field"
                          v-model="row.formData[cell.editable.field.name]"
                        ...
                        <v-text-field
                          v-else-if="showField('string', cell.editable.field)"
                          v-model="row.formData[cell.editable.field.name]"
                          :label="cell.editable.field.label"
                          :placeholder="cell.editable.field?.placeholder"
                          clearable
                          :name="cell.editable.field.name"
                          :class="[...cell.editable.field.class]"
                          v-mask="cell.editable.field.mask"
                        >
                          <template
                            v-if="cell.editable.field?.appendAction?.length"
                            v-slot:append-outer
                          >
                            <!-- <v-icon> {{ field.appendAction.icon }} </v-icon> -->
                            <v-tooltip
                              v-for="action in cell.editable.field.appendAction"
                              :key="action.label"
                              top
                            >
                        ...
                        <v-checkbox
                          v-else-if="showField('checkbox', cell.editable.field)"
                          v-model="row.formData[cell.editable.field.name]"
                          :label="cell.editable.field.label"
                          :name="cell.editable.field.name"
                          :class="'checkbox_' + cell.editable.field.name"
                          @change="
                            changeValue({
                              value: row.formData[cell.editable.field.name],
                              field: cell.editable.field,
                            })
                          "
                        ></v-checkbox>
                        <Datepicker
                          v-else-if="showField('date', cell.editable.field)"
                          v-model="row.formData[cell.editable.field.name]"
                          :field="cell.editable.field"
                          :label="cell.editable.field.label"
                        ></Datepicker>
                        <v-textarea
                          v-else-if="showField('textarea', cell.editable.field)"
                          v-model="row.formData[cell.editable.field.name]"
                          :label="cell.editable.field.label"
                          clearable
                          rows="1"
                        />
                        <Datetimepicker
                          v-else-if="showField('datetime', cell.editable.field)"
                          :label="cell.editable.field.label"
                          v-model="row.formData[cell.editable.field.name]"
                          clearable
                        />
                        <DropZone
                          v-else-if="showField('dropzone', cell.editable.field)"
                          :options="cell.editable.field.options"
                          v-model="row.formData[cell.editable.field.name]"
                          :formData="formData"
                          :field="cell.editable.field"
                          @addFiles="addFiles($event, cell.editable.field)"
                        />
                        <ColorPicker
                          v-else-if="
                            showField('colorPicker', cell.editable.field)
                          "
                          v-model="row.formData[cell.editable.field.name]"
                          :formData="formData"
                          :field="cell.editable.field"
                        ...
                        <DateRange
                          v-else-if="
                            showField('dateRange', cell.editable.field)
                          "
                          v-model="row.formData[cell.editable.field.name]"
                          :formData="formData"
                          :field="cell.editable.field"
                  ...
                  <template
                    v-else-if="
                      row.row[cell.value] && row.row[cell.value][0]
                        ? row.row[cell.value][0].addingShow
                        : false
                    "
                  >
                    <v-text-field
                  ...
                  <template v-else-if="cell.type === 'default'">
                  ...
                  <template
                    v-else-if="
                      cell.type === 'object' &&
                      Object.byString(row.row, cell.value) &&
                      Object.byString(row.row, cell.value)[0]
                        ? !Object.byString(row.row, cell.value)[0].addingShow
                        : false
                    "
                  >
                    <!-- <template
                  ...
                  <template v-else-if="cell.type === 'checkbox'">
                    <v-icon
                      @click="removeEditable(row, rowEditableIndex)"
                      color="red"
                    >
                      mdi-minus
                    </v-icon>
                  </template>
                  <template v-else-if="cell.type === 'index'">
                    {{ options.data.totalRows + rowEditableIndex + 1 }}
                  </template>
                  <!-- <template v-else-if="row.row[cell.value]">
                    {{ options.data.totalRows + rowEditableIndex + 1 }}
                  </template> -->
                </td>
              </tr>
            ...
            <template v-for="row in options.data.rows">
              <tr
                ...
                @contextmenu="openContext($event, row)"
                @click="openChildRow($event, row)"
                class="v-table-body-row v-table-body-row"
                ref="cellItems"
              >
                <!-- <th
                  :class="[
                ...
                <td
                  ...
                  :key="cellIndex"
                >
                  <template v-if="cell?.editable?.active">
                    {{ row.row.index }}
                  </template>
                  <template
                    v-else-if="
                  ...
                  <template v-else-if="cell.type === 'checkbox'">
                    <v-checkbox class="table-td-checkbox"></v-checkbox>
                  </template>
                  <template v-else-if="cell.type === 'index'">
                    {{ row.row.index }}
                  </template>
