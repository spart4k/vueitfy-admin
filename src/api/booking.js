...
export default class Booking {
  ...
  async get() {
    try {
      const { data } = await axios.get(
        `appointment/api/admin/getAppointmentsUsers?page=1&limit=99999`
      )
      console.log(data)
  ...
  async getServiceMasters(id) {
    try {
      const { data } = await axios.get(
        `users/api/admin/getServiceMasters?user_id=${id}`
      )
      return data
    } catch (error) {
      console.log(error)
  ...
  async update(requestData, id) {
    try {
      const { data } = await axios.patch(
        `appointment/api/admin/editAppointmentUser?appointment_id=${id}`,
        requestData
      )
      console.log(data)
  ...
  async delete(id) {
    try {
      const response = await axios.delete(
        `appointment/api/admin/delAppointmentUser?appointment_id=${id}`
      )
