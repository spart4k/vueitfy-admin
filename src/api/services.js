  ...
  async get() {
    ...
    try {
      const data = await axios
        .post(`users/api/getFirstNGenerationsOfChildren`, {
          n: 4,
          conditions: {
            floor: 0,
          },
  ...
  async create(chapter) {
    ...
    try {
      const { data } = await axios.post(
        `users/api/addOrEditServiceOrCategory`,
        chapter
      )
      const newChapter = data
      store.commit('alert/show', {
        type: 'success',
        content: `Услуга успешно добавлена`,
        duration: 2000,
      })
      if (!newChapter) {
        return null
  ...
  async delete(id) {
    try {
      const response = await axios.get(
        `users/api/deleteServiceOrCategory/${id}`
      )
      console.log(response)
      store.commit('alert/show', {
        type: 'success',
        content: `Услуга успешна удалена`,
        duration: 2000,
      })
    } catch (error) {
